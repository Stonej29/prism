...
- Endpoints live under `/api` (`web/routes/`): notes (list/detail/save/reprocess/retry/delete/edit-tags/rename/set-status), graph, tags, stats, find, ask, ideas (list/generate/rate/delete). In production `app.py` mounts the built `frontend/dist` as a SPA at `/` (via `PRISM_WEB_STATIC`, default `/app/static` in Docker).
