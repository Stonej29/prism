...
- **Review-status filtering**: the `list_notes_*` DB methods take an optional `status` (`db._status_clause`: `None` → hide `archived` from the default view, `"all"` → no filter, else exact match). `get_note_stats`/`NoteStats` carry `unreviewed`/`reviewed`/`archived` counts. The bot's `/inbox` (and TUI `inbox` / `f` filter cycle, CLI `--status`) surface the unreviewed queue (`list_inbox_notes`: every unreviewed note, archived excluded); the web graph keeps the full corpus (`gather_all_notes` uses `status="all"`) and dims archived nodes, with an inbox badge in the TopBar.
- **User-defined purpose** (primary single-select classification, above tags): the category set is **data**, not a hardcoded list — the `purposes` table (`name`, `description`, `sort_order`) is seeded with five defaults on first run, then fully user-editable via `/api/purposes` (CRUD) and the Settings "Purposes" panel (frontend reads it through the `usePurposes()` hook; `PURPOSE_VALUES` in `notes.py` is only the seed/fallback). Classification happens in the **cheap personalization pass** (call 2), which is handed the current category set (name + description) plus a compact few-shot of the reader's own past choices (`list_user_classified_notes`) — so new categories sort correctly and the AI learns from corrections without bloating context. Each note carries `purpose_source` (`"auto"` = AI-set, `"user"` = manually set/endorsed). The classifier applies a purpose **only the first time** (purpose+source both null); an already-classified note is never silently relabelled — `repersonalize`/profile edits leave it alone. **Re-classification is proposal-gated**: `NoteService.propose_reclassification()` (Settings "scan", `POST /api/purposes/scan`) re-runs the classifier over non-user notes and writes `repurpose` proposals (`proposals.py` `KIND_REPURPOSE`) for differences; approving applies + pins (`user`), rejecting pins the current value (`pin_purpose`) so the sweep stops re-proposing. User-pinned notes are always skipped.
