...
- Docker Compose publishes the web UI on all interfaces (`8000:8000`), reachable from your LAN. On a network-exposed bind it requires a login: on first run the UI prompts you to create an owner account, stored as a PBKDF2 hash under `runtime/` (path overridable via `PRISM_WEB_AUTH_FILE`). That first-run setup screen is open until an account exists, so create it promptly on a trusted network — or pin the credential up front with `PRISM_WEB_USERNAME`/`PRISM_WEB_PASSWORD` (which disables in-UI creation), or front the service with an authenticated reverse proxy. Login state is a signed session cookie; over HTTPS also set `PRISM_WEB_COOKIE_SECURE=1`. To run loopback-only without a login, set `PRISM_WEB_HOST=127.0.0.1` and bind the port to `127.0.0.1:8000:8000`. Keep it on a trusted LAN/VPN; never expose it directly to the public internet.
